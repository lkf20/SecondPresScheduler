...
- [ ] Investigate Time Off cancel UI bug: side panel can get stuck open and repeatedly prompt "discard changes" after canceling a request; fix close/reset panel state flow
...
- [ ] Dashboard Action Required banner: add per-shift quick actions in expanded details (deep-link to baseline schedule and/or school calendar for each issue)
...
- [ ] Assign Sub non-sub override V2: reconsider `assignment_type` semantics for non-sub overrides (currently stored as "Substitute Shift" for compatibility) and evaluate introducing a clearer assignment type model without breaking coverage/reporting behavior
...
- [ ] **Review: Floater + partial on two-room absence (e.g. 0.25 weight per room)** – Today `POST /api/sub-finder/assign-shifts` rejects the same `coverage_request_shift_id` being both `is_floater` and partial; contracts (ASSIGN_SUB_CONFLICT_RESOLUTION_CONTRACT Rule 2) and `lib/schedules/coverage-weights.ts` treat floater (0.5) and partial (0.5) separately—no combined 0.25. Before implementing: lock product semantics (shared vs independent time windows, how weights sum with other partials), update contracts, API validation/insert paths, centralized coverage math everywhere dashboards/weekly/sub-finder use it, DB if needed (optional explicit weight column), Assign Sub + Sub Finder UI, tests, and risk review (under/over-staffing, Phase 1 partial approximation, collision rules).
