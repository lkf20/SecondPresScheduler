...
export default function DaySelector({ selectedDayIds, onSelectionChange }: DaySelectorProps) {
      ...
      {daysOfWeek.map(day => {
        ...
        return (
          <div key={day.id} className="flex items-center space-x-2">
            ...
            />
            <Label htmlFor={`day-${day.id}`} className="cursor-pointer text-sm font-normal">
              {day.name.slice(0, 3)}
            </Label>
          </div>
        )
