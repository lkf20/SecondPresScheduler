...
export default function WeeklyScheduleGrid({ data }: WeeklyScheduleGridProps) {
  ...
  const handleCellClick = (
    ...
    setSelectedCell({ day_of_week_id: dayId, day_name: dayName, time_slot_id: timeSlotId, time_slot_name: timeSlotName })
  }
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr>
                ...
                {daysOfWeek.map((day) => (
                  <th key={day.id} className="border p-2 bg-muted font-medium text-center min-w-[200px]">
                    {day.name}
                  </th>
                ))}
              </tr>
