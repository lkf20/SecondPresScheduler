    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <FormField label="Day of Week" error={errors.day_of_week_id?.message} required>
          <Select
            value={dayOfWeekId || ''}
            onValueChange={value => setValue('day_of_week_id', value)}
          >
            <SelectTrigger>
              <SelectValue
                placeholder={daysOfWeek.length === 0 ? 'Loading days...' : 'Select a day'}
              />
            </SelectTrigger>
            <SelectContent>
              {daysOfWeek.length === 0 ? (
        ...
        <FormField label="Classroom" error={errors.classroom_id?.message} required>
          <Select
            value={classroomId || ''}
            onValueChange={value => setValue('classroom_id', value)}
          >
            <SelectTrigger>
