...
- Directors can keep a reusable baseline note per baseline cell and optionally customize the note for one specific date in Weekly Schedule.
- Weekly cell editing must stay simple for non-technical users: default to baseline note, with explicit controls for date-specific custom note or hiding note for that date.
- Weekly cell display should show one effective note (or none), while preserving baseline context in the editor.
...
- Add toggle: `Customize note for this date`.
- Turning toggle on both reveals custom note field and focuses cursor in that field.
- Custom note field is prefilled from baseline note when toggle first turns on (if baseline note exists).
- Add checkbox in custom mode: `Hide note for this date`.
...
- `customize_note_for_date`: UI boolean toggle.
- `hide_note_for_date`: UI boolean checkbox (only visible when customize is on).
- `weekly_custom_note`: UI text field (visible when customize is on and hide is off).
...
1. Customize off, no override row:
   - Effective note = baseline note.
2. Customize on, `override_mode='custom'`, note non-empty:
   - Effective note = weekly custom note.
3. Customize on, `override_mode='hidden'`:
   - Effective note = none (hidden for this date).
4. Baseline note empty + customize off:
   - Effective note = none.
...
1. Off -> On:
   ...
   - Prefill `weekly_custom_note` from baseline note if first activation and custom note empty.
   - Autofocus custom note field.
2. On + hide off -> hide on:
   - Disable custom note field.
   - Save as `override_mode='hidden'`.
3. On + hide on -> hide off:
   - Enable custom note field; keep last non-empty draft if available.
4. On -> Off:
...
5. Accessibility:
   - Toggle, checkbox, and textarea labels must be explicit and linked; focus must move predictably.
...
- Notes card layout:
  ...
  - Toggle: `Customize note for this date`
  - If toggle on:
    - Checkbox: `Hide note for this date`
    - Textarea (only enabled when hide is off)
...
- Customize on + custom note saves and displays in weekly cell.
- Customize on + hide note saves hidden override and displays no note.
...
- Off -> On prefill and autofocus.
- Custom -> hidden -> custom toggles preserve behavior.
- On -> Off removes override and reverts to baseline.
...
- Decision A (resolved): blank custom note vs explicit hide control.
  - Recommendation: explicit `Hide note for this date` is clearer and less error-prone than blank-note semantics.
- Decision B (resolved): prefill custom note when enabling customize.
  - Recommendation: Yes.
