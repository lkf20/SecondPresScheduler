...
export default function BaselineSchedulePage() {
  ...
  // Apply filters to data. Structural filters (days, time slots, classrooms) only limit which
  // rows/columns exist; we fill in all selected (classroom, day, slot). Collapse only when
  // slotFilterMode is 'select' and at least one staffing checkbox is unchecked.
      ...
      .map(classroom => {
        ...
        const days = daysWithSlots.map(day => ({
          ...
          time_slots: day.time_slots.filter(slot => {
            ...
            if (showAllSlots) return true
            if (!df.inactive && slot.time_slot_is_active === false) {
              return false
            }
